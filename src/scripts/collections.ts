import {
    folder_collection, file_collection, file, string_field, bool_field,
    enum_field, datetime_field, image_field, text_field, markdown_field,
    list_field, divider, object_field, number_field, all_collections
...
const icons = [
    ...
    { label: "Messenger", icon: "facebook-messenger", value: "facebook-messenger" },
...
const vesti =
    folder_collection("vesti", "Vesti", "newspaper", "src/content/vesti", [
        ...
        object_field("map", "Mapa", [
            ...
            ]).singular( "Marker").required(false)
        ]).required(false),
        ...
        list_field("timeline", "Tajmlajn",[
            ...
            list_field("videos", "Video IDs", [
                string_field("id", "ID")
            ]).singular("Video ID").required(false),
            list_field("slike", "Slike", [
                image_field("slike", "Slike")
            ]).singular("Slika").required(false),
            markdown_field("tekst", "Tekst")
        ]).add_to_top().required(false).i18n(false),
    ]).slug("{{fields.link}}").sort_by("pubDate");

const akcije =
    folder_collection("akcije", "Akcije", "timeline", "src/content/akcije", [
        string_field("link", "Link").i18n("duplicate"),
        string_field("title", "Naslov"),
...
const afere =
    folder_collection("afere", "Afere", "priority_high", "src/content/afere", [
        string_field("link", "Link").i18n("duplicate"),
        string_field("title", "Naslov"),
        markdown_field("body", "Tekst"),
    ]).slug("{{fields.link}}");
...
const stranice =
    file_collection("stranice", "Stranice", "description", [
        ...
        file("ostali_linkovi", "Ostali linkovi", "src/content/stranice", "ostali-linkovi.md", [
            string_field("ostali_linkovi", "Ostali linkovi"),
            string_field("najnovije_vesti", "Najnovije vesti"),
            list_field("linkovi", "Linkovi", [
                enum_field("ikonica", "Ikonica", icons.map(({value, label}) => [value, label])),
                string_field("naslov", "Naslov"),
                string_field("link", "Link").url(true)
            ]).singular("Link")
        ]),
...
const linkovi =
    folder_collection("linkovi", "Linkovi", "link", "src/content/linkovi", [
        string_field("title", "Naslov"),
